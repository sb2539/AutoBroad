...
if __name__ == "__main__":
    ...
    RPATH = PATH + data_name + "/"
    print(PATH)
    print(RPATH)
    with open(RPATH +data_name+".json", "r") as f:
        ucr_json = json.load(f)
    num_test = ucr_json[data_name]["datanum"]
    len_test = ucr_json[data_name]["datalen"]
    class_num = ucr_json[data_name]["classnum"]
    ...
    ucr_test_re = resize(ucr_test, ((num_test, len_test, 1)))
    teencoder = LabelEncoder()
    teencoder.fit(ucr_target_test)
    ucr_target_test = teencoder.transform((ucr_target_test))

    input_size = args.input_size
    stage_num = ucr_json["Arch"]["stage"]
    cell = ucr_json["Arch"]["cell"]
    dim = ucr_json["Arch"]["dim"]
    test_size = num_test
    ...
    infeature_list = ucr_json[data_name]["infeature"]
