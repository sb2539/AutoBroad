    ...
    def train(self, config: Configuration, seed: int = 0) -> float:
        with warnings.catch_warnings():
            ...
            if img_test_acc < test_acc:
                ...
                torch.save(model.state_dict(), PATH +str(num_save)+'ElectricDevices(new)64.pt')
                model.load_state_dict(torch.load(PATH + str(num_save) + 'ElectricDevices(new)64.pt'))
                correct = 0
                total = 0
                accuracy = []
                iter_acc = []
...
if __name__ == "__main__":
    ucr = fetch_ucr_dataset('ElectricDevices', use_cache=True, data_home=None, return_X_y=False)  # 5 class
    ...
    df_results.to_csv(PATH+"shapesall64.csv")
