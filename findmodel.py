...
if __name__ == "__main__":
    ...
    PATH = "/home/sin/PycharmProjects/please/save/FordB/"
    #RPATH = PATH + args.ucr + "/"
    print(PATH)
    #print(RPATH)
    data_name = "FordB"
    ...
    train_fix = np.concatenate(
        (train_dict["mk"], train_dict["trgad"],train_dict["trrp"]
         ,train_dict["ctwav"],train_dict["trmk"],train_dict["regad"],train_dict["remk"]), axis=1)
    test_fix = np.concatenate(
        (test_dict["mk"], test_dict["trgad"], test_dict["trrp"]
         , test_dict["ctwav"], test_dict["trmk"], test_dict["regad"], test_dict["remk"]), axis=1)
    best_test_acc = 0
    ...
    for i in range(3000):
        ...
        if img_test_acc < test_acc:
            ...
            torch.save(model.state_dict(), PATH +str(num_save) +data_name+'64(new).pt')
    ...
    model.load_state_dict(torch.load(PATH +str(num_save) +data_name + '64(new).pt'))
    criterion = nn.CrossEntropyLoss()
    correct = 0
    total = 0
    accuracy = []
