...
class Babroad:
    ...
    def train(self, config: Configuration, seed: int = 0) -> float:
        with warnings.catch_warnings():
            ...
            torch.save(model.state_dict(), PATH +str(candi) +'ElectricDevices_train(smac3).pt')
            ...
            if img_test_acc < test_acc:
                ...
                torch.save(model.state_dict(), PATH + 'ElectricDevices(smac3).pt')
...
if __name__ == "__main__":
    ucr = fetch_ucr_dataset('ElectricDevices', use_cache=False, data_home=None, return_X_y=False)  # 5 class
    ...
    ucr_test_re = resize(ucr_test, ((7712, 96, 1)))
